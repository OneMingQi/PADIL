...
from algorithms.rlkit.core import logger, eval_util
from algorithms.rlkit.data_management.env_replay_buffer import EnvReplayBuffer
from algorithms.rlkit.data_management.path_builder import PathBuilder
from algorithms.rlkit.policies.base import ExplorationPolicy
from algorithms.rlkit.torch.common.policies import MakeDeterministic
from algorithms.rlkit.samplers import PathSampler, VecPathSampler
from generator.policy import diffusion_actor_policy
    ...
    def __init__(
        ...
        if eval_sampler is None:
            if eval_policy is None:
                ...
                # print(type(eval_policy).__name__)
                if type(eval_policy).__name__ != "diffusion_actor_policy":
        ...
        for epoch in gt.timed_for(
            ...
            for steps_this_epoch in range(self.num_env_steps_per_epoch // self.env_num):
                if self.diffusion_steps_needed == False:
                    ...
                    actions, diffusion_actions = self._get_action_and_info(observations)
    ...
    def _try_to_eval(self, epoch):
        if self._can_evaluate():
            ...
            if (self.freq_saving <= 0) and (epoch < self.num_epochs - 1):
                ...
                if (int(epoch) % self.freq_saving == 0) or (
    ...
    def _get_action_and_info(self, observation):
        ...
        if self.diffusion_steps_needed == False:
            ...
            if not self._can_train():
                ...
                repeat_actions = np.repeat(
                    actions[:, np.newaxis, :], self.diffusion_n_timesteps, axis=1
                )

                return actions, repeat_actions

            actions, diffusion_actions = self.exploration_policy.get_actions(
                observation
            )
        ...
        if self.diffusion_steps_needed == False:
            ...
            if path_builder:
                self._current_path_builder[idx].add_all(
                    ...
                    absorbings=absorbing,
                    env_infos=env_info,
                    diffusion_actions=diffusion_action,
                )
            if add_buf:
                self.replay_buffer.add_sample(
                    observation=observation,
