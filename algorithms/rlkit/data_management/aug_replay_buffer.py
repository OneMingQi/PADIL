from algorithms.rlkit.data_management.env_replay_buffer import (
    EnvReplayBuffer,
)

# from gym.spaces import Box, Discrete, Tuple, Dict
import algorithms.rlkit.data_management.data_augmentation as rad
        ...
        if len(self.augs_funcs.keys()):
            for aug, func in self.augs_funcs.items():
                ...
                if "crop" in aug or "cutout" in aug:
                    ...
                    if "observations" in keys:
                        if self.cpc:
                            batch_data["observations_pos"] = batch_data[
                                "observations"
                            ].copy()
                            batch_data["next_observations_pos"] = batch_data[
                                "next_observations"
                            ].copy()
            ...
            if self.cpc:
                ...
                if "next_observations" in keys:
                    batch_data["next_observations_anchor"] = batch_data[
                        "next_observations"
                    ]
