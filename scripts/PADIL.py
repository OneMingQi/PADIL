...
from generator.trainer import diffusion_policy_trainer
...
def experiment(variant):
    """
    Run an experiment using the provided variant.

    Args:
        variant (dict): The experiment variant containing the necessary parameters.

    Returns:
        str: A signal indicating that the training is done.
    """
    # Load demo listings from a YAML file
    with open("experts/expert_data.yaml", "r") as f:
    ...
    policy_args = variant["diffusion_params"]
    ...
    trainer = diffusion_policy_trainer(
        policy=policy, q1=qf1, q2=qf2, env=env, args=policy_args
    )
